    ...
    std::uint32_t destination_stride)
{
    ...
    for (uint32_t y = 0; y < height; ++y)
    {
        ...
        for (uint32_t x16 = 0; x16 < width_over16; ++x16)
        {
            ...
            const uint8x8x2_t source_data = vld1_u8_x2(source_line);
            const uint8x8x2_t destination_data = vld1_u8_x2(destination_line);
   ...
   // Important - this must not be larger than the roi_width.
    ...
    for (uint32_t y = 0; y < height; ++y)
    {
        ...
        for (uint32_t x8 = 0; x8 < width_over8; ++x8)
        {
            ...
            const uint8x8x2_t source_data = vld1_u8_x2(reinterpret_cast<const uint8_t*>(source_line));
            const uint8x8x2_t destination_data = vld1_u8_x2(reinterpret_cast<uint8_t*>(destination_line));

            uint8x8x2_t result;
            result.val[0] = vbsl_u8(mask_1, source_data.val[0], destination_data.val[0]);
            result.val[1] = vbsl_u8(mask_2, source_data.val[1], destination_data.val[1]);

            vst1_u8_x2(reinterpret_cast<uint8_t*>(destination_line), result);
